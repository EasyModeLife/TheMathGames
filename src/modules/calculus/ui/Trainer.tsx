...
import { calculusEngine } from '../logic/engine'
...
export default function Trainer({ config, mode = 'game' as const, onFinish }: TrainerProps) {
  ...
  useMemo(() => {
    calculusEngine.init(config ?? { level: 1 })
  }, [config])

  return (
    <div>
      <h3>Cálculo · Entrenador</h3>
      <QAShell engine={calculusEngine} mode={mode} onFinish={onFinish} />
    </div>
  )
}
